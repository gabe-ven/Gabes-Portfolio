...
function Band({
  ...
  const photoTexture = useTexture(photoSrc ?? LANYARD_PNG);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current && card.current && band.current) {
      [j1, j2].forEach((ref) => {
        const body = ref.current as LerpBody;
        if (!body.lerped) {
      ...
      curve.points[1].copy((j2.current as LerpBody).lerped!);
      curve.points[2].copy((j1.current as LerpBody).lerped!);
      curve.points[3].copy(fixed.current.translation());
      // meshline geometry
      (band.current.geometry as THREE.BufferGeometry & { setPoints: (pts: THREE.Vector3[]) => void }).setPoints(
  ...
  const cardMap = photoSrc ? photoTexture : materials.base.map;
