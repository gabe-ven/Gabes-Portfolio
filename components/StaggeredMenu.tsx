...
export const StaggeredMenu = ({
  ...
  const prelayerColors = (() => {
    const raw = colors?.length ? colors.slice(0, 4) : ['#e8c4b8', '#D97D5B'];
    const arr = [...raw];
  ...
  return (
    <>
      {/* Separate fixed element so mix-blend-mode: difference blends against
          the root stacking context (page sections), not the panel wrapper */}
      <header className="sm-blend-header" aria-label="Main navigation header">
        <div className="sm-logo" aria-label="Logo">
          {logo}
        </div>
        <div className="sm-header-actions">
          <AnimatedThemeToggler className="sm-theme-toggle" />
          <button
            ref={toggleBtnRef}
            className="sm-toggle"
    ...
    >
      ...
      <aside id="staggered-menu-panel" ref={panelRef} className="staggered-menu-panel" aria-hidden={!open}>
        <button
          ...
          type="button"
        >
          <span className="sm-panel-close-line" />
          <span className="sm-panel-close-line sm-panel-close-line-v" />
        </button>
