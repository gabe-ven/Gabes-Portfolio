...
export interface LogoLoopProps {
  ...
  hoverSpeed?: number;
  /** When true, velocity snaps instantly (no ease) — best for infinite marquees */
...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    const applyTransform = () => {
      ...
      track.style.setProperty("--logoloop-offset-x", `${x}px`);
    ...
    const animate = (timestamp: number) => {
      ...
      const hover = hoverSpeedRef.current;
      const target =
        isHoveredRef.current && hover !== undefined ? hover : targetVelocityRef.current;
      ...
      const seqSize = seqSizeRef.current;
      if (seqSize > 0 && velocityRef.current !== 0) {
