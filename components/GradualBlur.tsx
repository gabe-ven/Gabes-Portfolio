...
const DEFAULT_CONFIG: Required<Omit<GradualBlurProps, 'width' | 'hoverIntensity' | 'preset' | 'onAnimationComplete'>> & Pick<GradualBlurProps, 'width' | 'hoverIntensity' | 'preset' | 'onAnimationComplete'> = {
  position: 'bottom',
  strength: 2,
  height: '6rem',
  width: undefined,
...
function useResponsiveDimension(responsive: boolean, value: string | undefined): string | undefined {
  ...
  useEffect(() => {
    ...
    const calc = () => setVal(value);
    const debounced = debounce(calc, 100);
    calc();
    window.addEventListener('resize', debounced);
    return () => window.removeEventListener('resize', debounced);
  }, [responsive, value]);
  return responsive ? val : value;
}
